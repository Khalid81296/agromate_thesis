            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <h5><span class="font-weight-bolder">জিপি/সলিসিটর:</span>
                        {{ App\Models\Writ_CaseRegister::findOrFail($info->id)->caseSF->user->name ?? 'এস এফ এখনো তৈরী করা হয়নি ' }}
                    </h5>
                    <h5>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif ( $caseActivityLog->message == 'মামলার এস এফ রিপোর্ট আপলোড করা হয়েছে')
                                        <h5>{{ $caseActivityLog->message ?? '-'}}</h5>
                                        <h5>ফাইলের নাম : "{{ $data[0]['writ__case_register'][0]['sf_report']}}"</h5>

                                        <a href="{{ asset('uploads/sf_report/'.$data[0]['writ__case_register'][0]['sf_report']) }}" target="_blank" class="btn btn-sm btn-success font-size-h5 float-left"> <i class="fa fas fa-file-pdf"></i><b>এফএফ প্রতিবেদন</b>
                                            {{-- <embed src="{{ asset('uploads/sf_report/'.$data[0]['writ__case_register'][0]['sf_report']) }}" type="application/pdf" width="100%" height="600px" />  --}}
                                         </a>
                                    @elseif ( $caseActivityLog->message == 'মামলার হিয়ারিং ফাইল আপলোড করা হয়েছে')
                                        <h5>{{ $caseActivityLog->message ?? '-'}}</h5>
                                        <h5>শুনানির তারিখ: {{ en2bn($data[0]['writ__case_hearing'][0]['hearing_date']) }}</h5>
                                        <h5>মন্তব্য: "{{ $data[0]['writ__case_hearing'][0]['hearing_comment']}}"</h5>
                                        <h5>ফাইলের নাম : {{ $data[0]['writ__case_hearing'][0]['hearing_file']}}</h5>

                                        <a href="{{ asset('uploads/order/'.$data[0]['writ__case_hearing'][0]['hearing_file']) }}" target="_blank" class="btn btn-sm btn-success font-size-h5 float-left"> <i class="fa fas fa-file-pdf"></i><b>মামলার হিয়ারিং ফাইল</b>
                                            {{-- <embed src="{{ asset('uploads/sf_report/'.$data[0]['writ__case_register'][0]['sf_report']) }}" type="application/pdf" width="100%" height="600px" />  --}}
                                         </a>
                                    @elseif ( $caseActivityLog->message == 'এফিডেভিট কমিটি মেম্বর নির্বাচন করা হয়েছে।')
                                        <h5>{{ $caseActivityLog->message ?? '-'}}</h5>
                                        <h5>মেম্বারস গণ:</h5>
                                        <b>@foreach($data as $key => $value)
                                            @foreach($value as $key_1 => $value_1)
                                                @foreach($value_1 as $key_2 => $value_2)
                                                     => {{ App\Models\AffidavitCommittee::where(['id' => $value_2["member_id"]])->pluck('name')->first()
                                                    }}<br> 
                                                @endforeach
                                            @endforeach
                                        @endforeach</b>
                                    @elseif ( $caseActivityLog->message == 'মামলার ফলাফল আপডেট করা হয়েছে')
                                        <h5>{{ $caseActivityLog->message ?? '-'}}</h5>
                                        @if ($data[0]['writ__case_register'][0]['is_win'] == 2)
                                            <h5>মামলার ফলাফলঃ পরাজয় </h5>
                                            <h5>পরাজয়ের কারণঃ " {{ $data[0]['writ__case_register'][0]['lost_reason'] ?? '-' }} "</h5>
                                        @else
                                            <h5>মামলার ফলাফলঃ জয়ী </h5>
                                        @endif
                                        <h5>বর্তমান ষ্ট্যাটাস:
                                            @if ($data[0]['writ__case_register'][0]['status'] == 1)
                                                নতুন চলমান!
                                            @elseif ($data[0]['writ__case_register'][0]['status'] == 2)
                                                আপিল করা হয়েছে!
                                            @elseif ($data[0]['writ__case_register'][0]['status'] == 3)
                                                সম্পাদিত !
                                            @endif
                                        </h5>
                                        <h5>
                                            মামলায় হেরে গিয়ে কি আপিল করা হয়েছে:
                                            @if ($data[0]['writ__case_register'][0]['is_lost_appeal'] == 1)
                                                হ্যা!
                                            @else
                                                না!
                                            @endif
                                        ...
                                        <h5>
                                            ...
                                            @if ($data[0]['writ__case_register'][0]['in_favour_govt'] == 1)
                                                হ্যা!
                                            @else
                                                না!
                                            @endif
