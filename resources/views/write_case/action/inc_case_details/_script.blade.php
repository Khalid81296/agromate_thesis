    ...
    $(document).ready(function() {
        ...
        $('#ajax-hearing-file-upload').submit(function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('writcase/action/file_store_hearing') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
        ...
        // Case Hearing Data and File Progress Bar
        $('#ajax-affidavit-committee-upload').submit(function(e) {
            e.preventDefault();
            // var bar = $('#bar');
            // var percent = $('#percent');
            var formData = new FormData(this);
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('writcase/action/affidavit_committtee_save') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: (data) => {
                    ...
                    // bar.width(percentVal);
                    // percent.html(percentVal);
                    $('#committeeMemberAddSuccess').show();
                    // window.location.href = SITEURL +"/action/details/"+"10";
                    console.log(data);
                    console.log(data.html);
                    $('.ajax').remove();
                    $('#caseCommitteeList').empty();
                    $('#caseCommitteeList').append('<label class="ajax" style="display:block !important;">' + data.html + '</label>')
                },
                /*complete: function(xhr) {
                   alert('File Has Been Uploaded Successfully');
                   // window.location.href = SITEURL +"/"+"ajax-file-upload-progress-bar";
        ...
        $('#ajax-file-upload').submit(function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('writcase/action/file_store') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
        ...
        $('#ajax-caseResult-upload').submit(function(e) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{ url('writcase/action/result_update') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
        ...
        $('#formSubmit').click(function(e) {
            ...
            $.ajax({
                url: "{{ url('writcase/action/forward') }}",
                method: 'post',
                data: {
        ...
        $('#sfCreateSubmit').click(function(e) {
            ...
            $.ajax({
                url: "{{ url('writcase/action/createsf') }}",
                method: 'post',
                data: {
                    case_id: $('#hide_case_id').val(),
        ...
        $('#sfUpdateSubmit').click(function(e) {
            ...
            $.ajax({
                url: "{{ url('writcase/action/editsf') }}",
                method: 'post',
                data: {
                    case_id: $('#hide_case_id').val(),
        ...
        //============//Hearing add=================//
        // Click hearing add button hide alert box
        $('#committee_member_add_button').click(function() {
            $('#committee_member_add_content').show();
            $('#committee_member_content').hide(1000);
            $('#committee_member_add_button').hide(1000);
        });
        $('#committee_member_add_button_close').click(function() {
            $('#committee_member_add_button').show();
            $('#committee_member_content').show(1000);
            $('#committee_member_add_content').hide(1000);
            // $('#hearing_add_button_close').hide(1000);
        });
         ...
         $('select[name="status_id"]').change(function(e) {
            ...
            var roleID =$('input[type=radio][name=group]:checked').val();
             $.ajax({
                ...
                url: '{{ url("/")}}/writcase/action/getDependentCaseStatus/' + roleID,
                type : "GET",
                dataType : "json", 
                  success: function(result) {
                    if (result.errors) {
